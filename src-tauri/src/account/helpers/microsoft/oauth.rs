...
pub async fn device_authorization(app: &AppHandle) -> XMCLResult<DeviceAuthResponseInfo> {
...
async fn fetch_xbl_token(app: &AppHandle, microsoft_token: String) -> XMCLResult<String> {
  let client = app.state::<reqwest::Client>();
...
async fn fetch_xsts_token(app: &AppHandle, xbl_token: String) -> XMCLResult<(String, String)> {
  let client = app.state::<reqwest::Client>();
...
async fn parse_profile(app: &AppHandle, tokens: &OAuthTokens) -> XMCLResult<PlayerInfo> {
...
pub async fn login(app: &AppHandle, auth_info: DeviceAuthResponseInfo) -> XMCLResult<PlayerInfo> {
  let client = app.state::<reqwest::Client>();
...
pub async fn refresh(app: &AppHandle, player: &PlayerInfo) -> XMCLResult<PlayerInfo> {
  let client = app.state::<reqwest::Client>();
