...
pub fn get_mod_metadata_from_jar<R: Read + Seek>(
...
) -> XMCLResult<LegacyForgeModMetadata> {
  ...
  if meta.is_empty() {
    return Err(XMCLError("len of LegacyForgeModMetadata is 0".to_string()));
  }
  Ok(meta.remove(0))
}

pub async fn get_mod_metadata_from_dir(dir_path: &Path) -> XMCLResult<LegacyForgeModMetadata> {
