...
> **關於 macOS 支援的說明：** 目前不提供 macOS 版本，因為需要 Apple Developer Program 會員資格（99 美元/年）進行程式碼簽名和公證。如果沒有適當的程式碼簽名，macOS 應用程式無法在現代 macOS 系統（macOS 10.15+）上執行，這是由於 Gatekeeper 安全要求。分發未簽名的應用程式會導致安全警告和潛在的法律/安全風險。如果您有興趣支援 macOS 開發，請考慮為專案做出貢獻或贊助。
